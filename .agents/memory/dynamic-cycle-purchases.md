...
**How to apply:** Keep normal Purchase behavior unchanged. For dynamic cycle purchases, bypass subscribed proposals, preserve scope/duplicate-purchase guards, include the selected barrier for barrier contracts, and remove it for Even/Odd. Generate trade options with the active account currency and choose symbols from the live supported volatility-market list rather than hardcoding either value.